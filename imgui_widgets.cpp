...
bool ImGui::TreeNode(ImStrv label)
{
    ...
    return TreeNodeBehavior(id, id, ImGuiTreeNodeFlags_None, label);
}
...
bool ImGui::TreeNodeEx(ImStrv label, ImGuiTreeNodeFlags flags)
{
    ...
    if (window->SkipItems)
        return false;
    ImGuiID id = window->GetID(label);
    return TreeNodeBehavior(id, id, flags, label);
}
...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiID storage_id, ImGuiTreeNodeFlags flags, ImStrv label)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
